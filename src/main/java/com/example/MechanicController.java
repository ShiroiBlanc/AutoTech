...
public class MechanicController {
    ...
    private void loadMechanicsFiltered(String searchTerm, String statusFilter) {
        try {
            // To ensure filtering uses the current computed availability (not a possibly stale DB column),
            // fetch all mechanics then filter in Java using calculateAvailability(...).
            List<Mechanic> mechanics = mechanicService.getAllMechanics();

            List<Mechanic> filtered = new java.util.ArrayList<>();
            String searchLower = searchTerm == null ? "" : searchTerm.toLowerCase();

            for (Mechanic m : mechanics) {
                String availability = mechanicService.calculateAvailability(m.getId());

                // Status filter
                boolean statusMatches = "All".equalsIgnoreCase(statusFilter) ||
                    (availability != null && availability.equalsIgnoreCase(statusFilter));

                // Search term match against name or specialties
                boolean searchMatches = searchLower.isEmpty() ||
                    (m.getName() != null && m.getName().toLowerCase().contains(searchLower)) ||
                    (m.getSpecialtiesAsString() != null && m.getSpecialtiesAsString().toLowerCase().contains(searchLower));

                if (statusMatches && searchMatches) {
                    // Ensure mechanic object has up-to-date availability set for downstream logic
                    m.setAvailability(availability);
                    filtered.add(m);
                }
            }

            updateMechanicViewList(filtered);
