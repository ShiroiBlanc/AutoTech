...
public class ServiceBookingController {
    ...
    // Specialty checkboxes for mechanic selection
    private List<CheckBox> mechanicSpecialtyCheckBoxes;
    ...
    private void loadBookingsFiltered(String searchTerm, String statusFilter, LocalDate dateFilter) {
        try {
            ...
            // Normalize status filter: convert display labels (e.g., "In Progress") to DB values (e.g., "in_progress")
            String normalizedStatusFilter = statusFilter;
            if (statusFilter != null && !statusFilter.equals("Active") && !statusFilter.equals("All")) {
                normalizedStatusFilter = convertToDatabaseStatus(statusFilter);
            }
            ...
            if (currentUser != null && currentUser.getRole() == User.UserRole.MECHANIC) {
                ...
                if (mechanic != null) {
                    bookings = bookingService.searchBookingsForMechanic(mechanic.getId(), searchTerm, normalizedStatusFilter, dateFilter);
                } else {
                ...
                bookings = bookingService.searchBookings(searchTerm, normalizedStatusFilter, dateFilter);
            }
    ...
    private void viewBookingDetails(ServiceBookingViewModel booking) {
        try {
            ...
            try {
                ...
                // Running total for service labor costs for this booking
                ...
                if (!services.isEmpty()) {
                    ...
                    // Add a Service Cost column
                    TableColumn<Map<String, String>, Double> serviceCostCol = new TableColumn<>("Service Cost");
                    serviceCostCol.setCellValueFactory(cellData -> {
                        String type = cellData.getValue().get("type");
                        double cost = getServiceLaborCost(type);
                        return new javafx.beans.property.SimpleDoubleProperty(cost).asObject();
                    });
                    serviceCostCol.setPrefWidth(140);
                    serviceCostCol.setCellFactory(col -> new TableCell<Map<String, String>, Double>() {
                        @Override
                    ...
                    servicesTable.getColumns().addAll(serviceTypeCol, serviceDescCol, serviceCostCol);

                    // Sum up service costs
                    for (Map<String, String> svc : services) {
                        String type = svc.get("type");
                        totalServiceCost += getServiceLaborCost(type);
                    }
                    serviceSection.getChildren().add(servicesTable);
                    // Show total service cost
                    Label totalServiceLabel = new Label("Total Service Cost: ₱" + String.format("%.2f", totalServiceCost));
                    totalServiceLabel.setStyle("-fx-font-size: 14px; -fx-font-weight: bold; -fx-text-fill: #2E7D32; -fx-padding: 10 0 0 0;");
                    serviceSection.getChildren().add(totalServiceLabel);
                } else {
            ...
            dialog.getDialogPane().setMinWidth(770);
    ...
    // Helper to get the labor cost for a service type (reads from service_labor_costs table)
    private double getServiceLaborCost(String serviceType) {
    ...
    customerComboBox.setPromptText("Search or select a customer");
    // Make the ComboBox editable so users can type to search
    customerComboBox.setEditable(true);
        
        try {
            List<Customer> customers = customerService.getAllCustomers();
            // Keep a master list to filter from when typing
            ...
            // Add editor listener to filter suggestions as the user types
            customerComboBox.getEditor().textProperty().addListener((obs, oldText, newText) -> {
                // Don't filter when a real Customer value was selected programmatically
                if (customerComboBox.getValue() != null && customerComboBox.getConverter().toString(customerComboBox.getValue()).equals(newText)) {
                    return;
                }
                ...
                if (filter.isEmpty()) {
                    ...
                    for (Customer c : allCustomers) {
                        String repr = (c.getName() + " " + c.getPhone() + " " + (c.getEmail() == null ? "" : c.getEmail())).toLowerCase();
                        if (repr.contains(filter)) {
                            filtered.add(c);
                        }
                    }
                ...
                // Show the popup so suggestions are visible
                if (!customerComboBox.isShowing()) {
                    customerComboBox.show();
                }
            });

            // If the user presses Enter in the editor, pick the first matching suggestion (if any)
            customerComboBox.getEditor().setOnKeyPressed(evt -> {
                switch (evt.getCode()) {
                    case ENTER:
                        ObservableList<Customer> currentItems = customerComboBox.getItems();
                        if (currentItems != null && !currentItems.isEmpty()) {
                            customerComboBox.setValue(currentItems.get(0));
                        }
                        break;
                    default:
                        break;
    ...
    private void setupMechanicSelection(GridPane grid, int row, boolean showSpecialties) {
        ...
        try {
            ...
            grid.add(mechanicComboBox, 1, row, 1, 1);

            if (showSpecialties) {
                // Add specialty checkboxes next to mechanic selector
                VBox specialtiesBox = new VBox(6);
                specialtiesBox.setStyle("-fx-padding: 0 0 0 10;");
                Label specLabel = new Label("Specialties:");
                specLabel.getStyleClass().add("form-label");
                specialtiesBox.getChildren().add(specLabel);

                mechanicSpecialtyCheckBoxes = new java.util.ArrayList<>();
                String[] availableSpecialties = new String[]{
                    "General Maintenance",
                    "Engine Specialist",
                ...
                for (String s : availableSpecialties) {
                    CheckBox cb = new CheckBox(s);
                    cb.setDisable(true); // read-only in booking dialog
                    mechanicSpecialtyCheckBoxes.add(cb);
                    specialtiesBox.getChildren().add(cb);
                }

                grid.add(specialtiesBox, 2, row, 1, 4);

                // When mechanic is selected, auto-check specialties
                mechanicComboBox.valueProperty().addListener((obs, oldMech, newMech) -> {
                    // Clear all
                    for (CheckBox cb : mechanicSpecialtyCheckBoxes) cb.setSelected(false);
                    if (newMech != null) {
                        java.util.List<String> specs = newMech.getSpecialties();
                        if (specs != null) {
                            for (CheckBox cb : mechanicSpecialtyCheckBoxes) {
                                for (String s : specs) {
                                    if (cb.getText().equalsIgnoreCase(s.trim())) {
                                        cb.setSelected(true);
                                        break;
                                    }
                                }
    ...
    private void setupPartsSelection(GridPane grid, int row) {
        ...
        // Total cost label (will show parts + service labor)
        Label totalLabel = new Label("Total Parts Cost: ₱0.00");
        totalLabel.setStyle("-fx-font-weight: bold; -fx-font-size: 14px;");
        grid.add(totalLabel, 0, row + 4, 3, 1);

        // Helper to recompute and show combined total
        Runnable recomputeTotals = () -> {
            double partsTotal = selectedPartsList.stream()
                .mapToDouble(BookingPart::getTotalCost)
                .sum();

            // Compute service total from current servicesList
            double serviceTotal = 0.0;
            for (Map<String, String> svc : servicesList) {
                String type = svc.get("type");
                serviceTotal += getServiceLaborCost(type);
            }

            double combined = partsTotal + serviceTotal;
            totalLabel.setText(String.format("Total (Parts + Services): ₱%.2f", combined));
        };

        // Update total when parts list changes
        selectedPartsList.addListener((javafx.collections.ListChangeListener<BookingPart>) c -> recomputeTotals.run());
        // Update total when services list changes (when user adds/removes services while creating booking)
        servicesList.addListener((javafx.collections.ListChangeListener<java.util.Map<String, String>>) c -> recomputeTotals.run());
    }
    ...
    private void showBookingDialog(ServiceBookingViewModel existingBooking) {
        try {
            ...
            // Create a ScrollPane to handle overflow; we'll keep the form spacing but constrain viewport height
            ScrollPane scrollPane = new ScrollPane();
            scrollPane.setFitToWidth(true);
            // Keep a reasonable viewport height so a vertical scrollbar appears when content overflows
            scrollPane.setPrefViewportHeight(480);
            
            // Create the form grid with the original, more generous spacing
            GridPane grid = new GridPane();
            ...
            setupMechanicSelection(grid, 4, existingBooking != null);  // Start at row 4 to leave space; show specialties only when editing
            ...
            // Set up the dialog pane: put the grid inside the scroll pane so content scrolls
            scrollPane.setContent(grid);

            DialogPane dialogPane = bookingDialog.getDialogPane();
            dialogPane.setContent(scrollPane);
            // Keep dialog width but reduce preferred height so scrollbar is used while preserving layout
            dialogPane.setPrefSize(900, 650);
            dialogPane.setMinSize(800, 520);
