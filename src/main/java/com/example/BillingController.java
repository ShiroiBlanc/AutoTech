...
public class BillingController {
                ...
                try {
                    ...
                    if (!services.isEmpty()) {
                        ...
                        for (java.util.Map<String, String> service : services) {
                            ...
                            double labor = getServiceLaborCost(type);
                            totalServicesCost += labor;
                            Label serviceLabel = new Label("• " + type + " — ₱" + String.format("%.2f", labor));
                        ...
                        Label serviceTotalLabel = new Label("Total Services: ₱" + String.format("%.2f", totalServicesCost));
                        serviceTotalLabel.setStyle("-fx-font-weight: bold; -fx-font-size: 11px; -fx-padding: 5 0 0 0;");
                        infoGrid.add(serviceTotalLabel, 1, 3);
                    }
                } catch (Exception e) {
        ...
        try {
            ...
            if (booking != null) {
                try {
                    ...
                    if (!services.isEmpty()) {
                        double totalServicesCost = 0.0;
                        htmlContent.append("<div style='margin-bottom:10px;'><table style='width:100%; border-collapse: collapse; font-size:12px;'>");
                        htmlContent.append("<tr style='background-color:#f0f0f0;'><th style='text-align:left; padding:6px; border:1px solid #ddd;'>Service</th><th style='text-align:right; padding:6px; border:1px solid #ddd;'>Price</th></tr>");
                        for (java.util.Map<String, String> service : services) {
                            String type = service.get("type");
                            double labor = getServiceLaborCost(type);
                            totalServicesCost += labor;
                            htmlContent.append("<tr>");
                            htmlContent.append("<td style='padding:6px; border:1px solid #ddd;'>").append(type).append("</td>");
                            htmlContent.append("<td style='text-align:right; padding:6px; border:1px solid #ddd; font-weight:bold;'>₱").append(String.format("%.2f", labor)).append("</td>");
                            htmlContent.append("</tr>");
                        }
                        htmlContent.append("</table>");
                        htmlContent.append("<p style='margin:6px 0 0 0; text-align:right; font-weight:bold;'>Total Services: ₱").append(String.format("%.2f", totalServicesCost)).append("</p>");
                        htmlContent.append("</div>");
                    }
    ...
    private double getServiceLaborCost(String serviceType) {
        ...
        try (Connection conn = DatabaseUtil.getConnection();
            ...
            if (rs.next()) {
                double v = rs.getDouble("labor_cost");
                rs.close();
                return v;
            }
            rs.close();
