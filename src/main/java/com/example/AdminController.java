...
public class AdminController {
    ...
    // Adjusted dialog width so action buttons are visible but not too wide
    mainBox.setPrefWidth(670);
    ...
    // Actions column width to accommodate Edit + Delete
        ...
        actionsCol.setCellFactory(col -> new TableCell<ServiceLaborCost, Void>() {
            ...
            private final HBox box = new HBox(6, editButton, deleteButton);
            {
                ...
                deleteButton.setOnAction(e -> {
                    ...
                    confirm.setHeaderText("Delete Service Labor Cost");
                    confirm.setContentText("Are you sure you want to delete the labor cost for '" + cost.getServiceType() + "'?");
                    confirm.showAndWait().ifPresent(btn -> {
                        if (btn == ButtonType.OK) {
                            // Delete from DB
                            try (Connection conn = DatabaseUtil.getConnection();
                                 PreparedStatement stmt = conn.prepareStatement(
                                     "DELETE FROM service_labor_costs WHERE service_type = ?")) {
                                stmt.setString(1, cost.getServiceType());
                                int rows = stmt.executeUpdate();
                                if (rows > 0) {
                                    // Refresh list
                                    loadLaborCostsFromDatabase(laborCostList);
                                    showAlert(Alert.AlertType.INFORMATION, "Deleted", "Labor cost deleted.");
                                } else {
                                    showAlert(Alert.AlertType.ERROR, "Delete Failed", "No record deleted. It may have already been removed.");
                                }
                            } catch (SQLException ex) {
                                ex.printStackTrace();
                                showAlert(Alert.AlertType.ERROR, "Database Error", "Failed to delete: " + ex.getMessage());
                            }
                        }
                    });
                });
    ...
    // Dialog pane size adjusted to be smaller
